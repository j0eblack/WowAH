...
def get_hellfire(token):
    search = "https://eu.api.blizzard.com/data/wow/auctions/commodities?namespace=dynamic-eu&locale=en_GB&access_token=" + token
    response = requests.get(search)
    return response.json()["auctions"]

#response = create_access_token('ab421c0dbfb841ad8fb89d6259976fdb', 'lC7Bdzq8zPHd9feowtaBXN9dhcJ5KVvF')
#token = response['access_token']

hellfire_auctions = get_hellfire('EUJ6LQSZSzaMscfbwrfl4NZCbcH2xt8c4k')
auction_df = pd.DataFrame( hellfire_auctions )

# Expand the item column'EUJ6LQSZSzaMscfbwrfl4NZCbcH2xt8c4k'
